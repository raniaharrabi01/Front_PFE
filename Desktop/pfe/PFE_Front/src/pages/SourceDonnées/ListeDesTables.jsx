...
const ListeDesTables = ({ DataBaseConnexion, ListeTablesAndColumns }) => {
  ...
  return (
    <div>
      <h3 className="font-medium text-[#4169E1] dark:text-[#4169E1]" style={{ paddingLeft: '28px', fontWeight: '600' }}> La liste des tables dans la base de données {DataBaseConnexion.base_name} : </h3>
        ...
        {ListeTablesAndColumns &&
          Object.entries(ListeTablesAndColumns).map(([table, columns]) => (
            <li key={table}>
              ...
              {selectedTable === table && (
                <ListColumns columns={columns} tableName={table} DataBaseConnexion={DataBaseConnexion} />
              )}
            </li>
          ))}
      </ul>
